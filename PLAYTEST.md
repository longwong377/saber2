...
| Note | Mark | Where |
|---|---|---|
| Companions — a persistent personal companion, twelve kinds, orders, growth, customisation, in the hangar and on the ships | 🔎 | `companions` (18) — the sim is whole: twelve kind ROWS with nothing switching on a name, a four-rung ladder that carries NO multiplier field at all (what grows is leash, order set and how long an order survives you), six orders driven on a real body in a mode with NO CommandDirector, a durable Kennel with its own fold, four two-sided tempers priced net ≤ 0, the four colour slots `buildQuadruped` has accepted for months and never been handed, and it boards the transport without appearing on the manifest `keep()` judges. **Eight of twelve kinds have a body**; the hangar representation is not built. See the row below |
...
| Three saber forms — the single blade untouched, a double-bladed saberstaff, and dual wielding, each with its own moves and its own trade | ✅ | `saberforms` (12) — **and the second blade was not in the game.** `World._bladeEntries` pushed `p.saber` and nothing else and `_resolveBlades` solved `p.saber` and nothing else, so the off blade landed 0 contacts and 0 deflections at every distance from 1.0 m to 2.6 m, and `offDamage`, `Sidearm.blades()` and `frontBlade()` had no caller anywhere; every clause below was unmeasurable because only one blade ever did anything. Five lines of World.js later, all five are measured. **MORE REACH**: furthest contact on a real body 1.82 m one blade → 2.08 m staff (+14%), bought with 0.30 m of real metal between the pommels and a `STAFF_SLASH.lunge` that was dead code, and paid for with 21% slower guard recovery. **THE SPIN BARRIER**: 11 of 24 bolts land with it down, 0 of 24 with it up — it failed twice first, once spinning in the horizontal plane coplanar with a flat bolt and once catching twice as much at 30 Hz as at 60, which is protection that scales with framerate. Hands on hilt 0, a push mid-spin costs 16 Force and leaves it turning. **THROW ONE, KEEP ONE**: 208 contacts with both, 84 with the shoto in the air, and the flying blade still cuts. **TWO-TEMPO**: the gap to the next landed strike drops 0.767 s → 0.517 s at the lower quartile. **AGAINST FOUR AT ONCE** the pair does +86% of the cut work — and answers 2.00 bodies a half-second against the single blade's 2.08, so it out-works one blade and does not out-SPREAD it, which the check reports rather than hides. The single blade is held against a 600-frame recording: 9 600 numbers, drift 0.00e+0, and Saber.js and Combat.js gained zero lines |
