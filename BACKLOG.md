...
None of these is on a list. They came out of auditing the V3 list against the
shipped code, and they are here so they are not lost.

> This section said "Neither of these… Both came out of" for as long as it had
> five rows, and 8.5 carried BOTH an `open` verdict and the ✅ that closed it in
> the same State cell — five cells in a four-column table, so it rendered as an
> item you could not tell the state of by reading it. Prose counts drift the way
> HANDOFF §1's table drifts; the row is the record, not the sentence over it.
