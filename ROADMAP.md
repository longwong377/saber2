...
| Six group composers | `Levels.js:485, 540, 613, 666, 973, 1240` | `strewGround`, `strewWrecks`, `roof`, `works` (a whole parameterised room, already built for two depths), `cut` (a whole mine — **orphaned, whole, callable**), `templeColonnade` |
...
| A live hook for a run object | `Levels.js:326` | `groundMight()` already reads `world.run.tier` and `world.run.boons` — for a `run` that no longer exists |
