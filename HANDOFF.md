...
1. **LIGHT** — light moods per room and hour (the cantina at 02:00 is not the cantina at noon), key-light shadows on bodies, emissive surfaces lighting the floor near them. `StationLight.js`. **BUILT** — `MOOD` is thirteen rooms by four hour bands (night 22–05, dawn, day, evening), applied by `placeUnder` and crossfaded over 2 s at a door; it owns the key, rim, ambient, fill and the strips' COLOUR and reads the three files that already write here (`Morning` the strips' intensity, `StationWar` their red, `StationLife.stepDip` the surge). THE SHADOWS WERE THE PROBLEM: `Props.addStatic` casts on every merged mesh, so the drum stood inside its own ceiling's shadow on every deck at every hour and the key did nothing — the shell and the rooms no longer cast, the engine's cascade 0 (the only directional light the cel patch reads a shadow for) is turned onto the mood's own bearing at a 1024 map with cascades 1 and 2 off, and a once-a-second sweep gives the shadow pass the nearest ≤ 40 bodies and props; off at quality 'low'. 648 emissive fixtures are found on deck 40 by binning the merged meshes' `strip`/`screen`/`status` vertices into 2.2 m cells: the nearest twelve within 30 m get a pooled `PointLight` (six at 'medium', none at 'low'), and all 638 within reach of a floor get an additive radial glow decal, one merged sheet per room. `tools/checks/stationlight.mjs`, `tools/_lightshot.mjs`.
2. **MOTION AND PARTICLES** — doors that open, fans that turn, screens that flicker, steam from the galley, litter drifting in the atrium's draught, dust motes in light, sparks at the forge, spray at the wet grating. `StationMotion.js`. **BUILT** — sliding leaves in every doorway the kit actually cuts (`kit.doorway`, recorded in `StationKit.walls`/`arcFront`; 28 pairs over three decks, none over 6.5 m, collider disabled while open); twelve fans; ±5% flicker at 8–12 Hz on every Holonet screen and card feed with a 0.2 s dropout a minute; steam at #15's pass, #16's ranges and #39's washers, smoke over #17's stalls while the ambient cook works, sparks at #10's bench, spray at #49's grating; twenty seeded scraps on a seeded wind field round the void; dust in the light shaft; the chandelier's rings, eight pendant lamps, four blinking route maps and #19's projector cone. 10–18 extra draws a deck, 0.03–0.11 ms a frame. `tools/checks/stationmotion.mjs`, `tools/_motionshot.mjs`.
3. **GESTURES** — bodies that lean on rails, drink, argue with their hands, point, wave a friend over, scratch, check a wrist. `Gestures.js`. **BUILT** — sixteen rows in `GESTURES`, each a short pose laid on the gait through the rig (`solveIK` on arm/fore, the trunk and head quaternions, the pelvis with the feet kept planted), blended in and out over `BLEND` = 0.3 s by one photograph-and-slerp-back in `layer()`. One hook: the last line of `StationLife.stepStanding`. Chosen off `h2(slot, roll)` exactly as `posture` is — never a clock, never `Math.random`. Context: a rail or a wall found in the physics broad phase by SIZE (a box whose top stands 0.86–1.28 m over the floor and is thin in one axis is a rail; taller is a wall), a partner within 1.6 m and facing for an argument, a same-quarter walker passing for a beckon, and a cup — a standing body in a bar is given one and sips every 8–15 s. A Vorlon does none of it, a Pak'ma'ra sniffs, a Drazi squares up instead of arguing. Walkers, sitters, guards, mission walkers and `__stationTouched` bodies are left alone. `tools/checks/gestures.mjs`.
4. **VOICE AND SCORING** — synthesised speech-like cadence for the tannoy and barks (syllables, pitch contour, a voice per species), and music that scores what you are doing (a fight, a chase, the vigil, the market) not only where you stand. `Voice.js`, `Music.js`. **BUILT** — `src/game/Voice.js`: `speak(text, voice, opts)` splits a line into syllables (vowel runs are nuclei, a run between two gives its last consonant to the next, silent -e folded, digits read as digits), gives each nucleus a real formant pair off the cardinal chart, and lays a sentence contour over them — a statement falls 1.06 → 0.86, a question lifts its last third to 1.28, long words take a stress. Eight voices, `f0` in Hz: human 118, drazi 86 (clipped, rate 1.30), minbari 196 (slow 0.70, breathy), centauri 150 (bend 0.22), narn 78 (sub-octave), vorlon 132 (a chord, rate 0.62), droid 240 (square, ring 2.71), tannoy 122 (band-limited 320–3400 Hz, 85 ms slap). The other seven species are DERIVED off a hash of the name, never defaulted. One `audio.shape` per line — two or three sources with automation on them, one voice out of the pool, however long the sentence — queued per voice so a tannoy never talks over itself, and the score ducks 6 dB while it does. `Music.js` gains nine SCORE states (idle/market/chase/fight/vigil/alert/sleep/win/loss), each a style, a tempo, an intensity and four layers, crossfaded over two bars ON THE BEAT and derived in `stepScore` from what the station already knows. `tools/checks/voice.mjs`.
...
20. Curved ring rooms that follow the drum, with windows onto the promenade glass. **BUILT** — every box-walled outer room is a SECTOR of the drum (`StationKit.sectorOfPlace`, one table): the front chords of the ring at `roomR` (`arcFront`), the side walls RADIAL (`radialSide`, turned ±(90° − th), th = asin(w/2 / 81)), the back an arc of the inner circle at `roomR − d` (`arcBack`). The back is `(81 − d) / 81` of the front: the cantina 26 → 18.3 m, Arrivals 46 → 34.6 m. Floor and lid are clipped strips on one collider; the soffit's and floor's dressing, the ribs, `furnish` and fourteen builders' own rows read the same numbers. `station.mjs` "side walls converge" builds each of the 24 wedges alone and holds every collider and loose body against it.

**Ten holes:**
1. Nobody has measured a frame on a real GPU.
2. Rooms are rectangles inboard of a curved ring. **CLOSED** — see cool 20.
3. No windows between rooms (rule 5 is a wall with a door). **CLOSED** — where two PUBLIC outer rooms are neighbours on a deck (facing side walls within 12.5 m of arc, no spine between) each cuts a 2.4 m pane, sill 1.0, head 2.4, in the wall that faces the other, both at ONE radius derived from the pair (`sectorOfPlace` `panes`, a third of the shorter room's depth in from the ring), glazed with the kit's glass as a static box and lit over the head on both faces. Five pairs: cantina ↔ the Pit (11.9 m apart), the Pit ↔ the Wheelhouse (3.5), Holo-theatre ↔ food court (7.3), Arrivals ↔ arboretum (9.6), medbay ↔ bacta ward (8.1); `station.mjs` "share a window". NOT a skin window on the back wall: the outer band hangs INBOARD of the ring walk (front at r = 81, back toward the axis), and the skin at r = 90 is on the far side of the 9 m walk — no back wall touches it. The Vorlon's door (6 m wide) and the round rooms (arena, chapel, comms, brig, reactor) are not sectors.
