...
5. **THE STAGE** — the drum's symmetry broken: deck 48 gets a dark abandoned quarter (sealed rooms, flickering strips, a warren of service corridors you can get lost in), deck 44 the rich deck, deck 40 the workers' street; some rooms huge, some tiny. `StationPlan.js`, `StationKit.js`. **BUILT** — `src/game/Stage.js` (the maze generator, the flicker, the stash), seven new places and one moved one:

   - **THE ABANDONED QUARTER, deck 48, bearings −60° to −12°** (48 degrees, 68 m of ring). `#62 The old fitters` is the one front on the arc that opens — somebody cut the welded plate and folded it back; `#63 The sealed mess` and `#64 The fallen soffit` are welded shut (`sealed: true`) and entered through cuts in the side walls, chained back to `#62`. Each has `heads: 0` and one flickering strip on its OWN cloned material, stepped by `Stage.stepStage` off a 32-slot seeded pattern, so no two are in step. **The Brig (#47) moved to make it** — the whole of deck 48's ring was measured with `tools/_stagefit.mjs` and this was the only stretch long enough; the cell block would not fit in any remaining gap on the ring (a deep room is wider at the back than at the front and both neighbours are deep), so it stands on the balcony at 137° instead, keeping its 22 × 20.
   - **THE RING IN FRONT OF IT IS DEAD**: no lit floor channel, no coffered soffit, no status lamps; rusted seam plates and capped conduit instead, and two `dead: true` fixtures — `The stripped kiosk` (303.7°) and `The quarter barricade` (343°) — which replaced a shopfront and a market that were standing in the middle of it. `The fab bench` moved to 211° off the Brig's old door arc.
   - **THE WARREN (#65)**, a new `spine` band — a block of plate between the balcony and the rooms. 7 × 9 cells on a four-metre grid, 2.2 m corridors between 1.8 m blocks of plant, 63 cells, 7 real dead ends, a valve room at the middle with a crate that pays 320 credits ONCE (`StationSave.stash`). Its spine exit is on the balcony walk; its ring exit comes up through the back of `#64`. Every wall is a collider; `floorAt` is untouched.
   - **BIG AND TINY**: `#7 Arrivals hall` 6.4 → 12 m with a mezzanine round its concourse side; `#48 Reactor hall` scaled up (eighteen catwalk segments the whole 30 m, a landing every third, an inner colonnade off the core). Three tiny rooms, 4 × 4 × 2.6, one per deck: `#66 Maintenance closet 48-9` (−52.5°, a ladder to a crawlspace hatch), `#67 The shrine niche` (44 at 82°), `#68 The ticket booth` (40 at 196°).
   - **THE THREE CHARACTERS** go through `dressWallRun`, per deck, not per room: 44 gets gilt, banners and a ribbon of strip and the ring gets planters; 40 gets awnings, chalk boards, crate stacks and a hand cart on the ring; the dead quarter gets rust, seams, dropped panels and lamps that are out. Gated on `kit.dressStreet` so a chapel does not get an awning. `StationKit.noteDress` records what was actually laid and `station.mjs` measures the difference.
   - **THE LIGHT IS NOT THIS LANE'S**: `StationPlan.DARK_ARC` is copied to `st.stage.darkArc = {a0, a1, k}` and nothing else here touches a lamp.
   - Five new checks in `tools/checks/station.mjs`; the doorway walk and the walk-the-world drop gate now honour `sealed` and enter at the plan's `cuts`. `tools/_stageshot.mjs` is the contact sheet, `tools/_stagefit.mjs` the bearing instrument.
