...
function walls(kit, M, w, d, h, opts = {}) {
  ...
  if (!open.has('front') && kit.arc) {
    ...
    /* V20 lane 2: the one place that knows a doorway was cut — `StationMotion`
     * hangs two sliding leaves in it; an open front, a glass `arcFront` or a
     * platform never reaches this line and gets no door. A sealed room's
     * plate stands in it, so it gets none either. */
    if (!opts.sealed) kit.doorway = { gap, z: -d / 2 - t / 2, t, head: Math.min(2.6, h) };
  }
}
