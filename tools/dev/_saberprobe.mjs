...
const info = () => page.evaluate(() => { const p = window.SABER.world.player; return { species: window.SABER.world.settings.species, stature: p.stature, stand: p.limbs?.stand, fp: !!p.camera.firstPerson, eye: +(p.eyeHeight ?? 0).toFixed(2), camY: +(p.camera.pos.y - p.position.y).toFixed(2), set: p.saberSet || p.set || null }; });
await page.mouse.click(640, 360);
await frames(45);
await page.keyboard.down('KeyW'); await frames(8); await page.keyboard.up('KeyW'); await frames(12);
...
await frames(10);
await page.keyboard.press('KeyV'); await frames(12);
