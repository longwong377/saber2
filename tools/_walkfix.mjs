...
const { wayPlacesOn, headcount, LIVE_RADIUS } = await import('./checks/../../src/game/StationLife.js');
  ...
  try {
    ...
    for (const hour of hours) {
      ...
      for (const p of ways) {
        ...
        for (let i = 0; i < n; i++) {
          /* the same scatter `slotIn` uses, approximated by the footprint */
          const d = Math.hypot(p.x - px, p.z - pz);
          if (d <= LIVE_RADIUS) { declared++; }
        }
      }
      ...
      for (const [, b] of life.live) {
        ...
        seen.set(b, { x: b.position.x, z: b.position.z, path: 0, lx: b.position.x, lz: b.position.z, t0: b.wayTrips | 0 });
      }
      ...
      const rows = [];
      for (const [b, s] of seen) {
        if (b.disposed || b.dead) continue;
        rows.push({ net: Math.hypot(b.position.x - s.x, b.position.z - s.z), path: s.path, trips: (b.wayTrips | 0) - s.t0 });
      }
