...
export async function run({ check, assert, THREE }) {
  ...
  check('seated: six bodies sit in the cantina at 22:00 inside 60 s, one to a chair, and stand back up', async () => {
    ...
    try {
      ...
      const held = new Set();
      /* EVERY BODY HOLDING ONE, not only the pool's: the duel's acolyte at
       * his booth (`world.enemies`), the player, and the STANDING drinkers'
       * cups (`Gestures.js`) — a cup in a hand is not a cup in the air. */
