...
for (const [file, src] of mods) {
  ...
  /* Minified for the artifact build: the hosted page has a 16MB cap and the
   * plain pack is 16.5. Comments are most of this codebase by design, so the
   * cut is large. Specifiers survive: esbuild transform keeps import strings
   * byte-identical, and the self-check below re-reads every one. */
  try {
    const { transformSync } = await import('esbuild');
    s = transformSync(s, { minify: true, format: 'esm', target: 'es2022' }).code;
  } catch (e) { console.warn('minify skipped for', rel(file), e.message?.slice(0, 80)); }
